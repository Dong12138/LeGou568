//
//  ReleaseZhaopinViewController.m
//  Lg
//
...
@interface ReleaseZhaopinViewController ()<ReleaseViewDelegate,FuliDelegate,UIActionSheetDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate>
@property (weak, nonatomic) IBOutlet UITableView *tableview;
@property (nonatomic,strong) NSArray *homeInfoArr;
...
@implementation ReleaseZhaopinViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    _homeInfoArr = @[@"分类",@"标题",@"价格",@"地址",@"配置",@"房屋说明",@"联系人",@"手机号",];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.row == 0) {
        ...
        if (_fenleiStr.length!=0) {
            ...
            label.text = _homeInfoArr[indexPath.row];
        }
    ...
    if (indexPath.row == 1||indexPath.row == 3) {
        ...
        tf.placeholder = _homeInfoArr[indexPath.row];
        return cell;
    }
    if (indexPath.row == 2) {
